...
mod tests {
    ...
    fn setup_e2e_pair() -> (
        ...
        [u8; 32], [u8; 32],  // peer IDs
        QueueId, QueueId,     // queue IDs
    ) {
        ...
        (alice_mgr, bob_mgr, bob_peer_id, alice_peer_id, alice_queue, bob_queue)
    }
    ...
    fn full_e2e_message_flow_through_envelope() {
        ...
        let plaintext = process_incoming_message(
            &mut bob_mgr,
            &alice_peer_id,
            &envelope,
    ...
    fn bidirectional_e2e_through_envelopes() {
        ...
        // Alice → Bob
        let env1 = prepare_outgoing_message(
            &mut alice_mgr, &bob_peer_id, &alice_queue, b"Hey Bob",
        ).unwrap();
        let pt1 = process_incoming_message(&mut bob_mgr, &alice_peer_id, &env1).unwrap();
        assert_eq!(pt1, b"Hey Bob");

        // Bob → Alice
        let env2 = prepare_outgoing_message(
            &mut bob_mgr, &alice_peer_id, &bob_queue, b"Hey Alice",
        ).unwrap();
        ...
        let env3 = prepare_outgoing_message(
            &mut alice_mgr, &bob_peer_id, &alice_queue, b"E2E ratchet works!",
        ).unwrap();
    ...
    fn envelope_ciphertext_is_not_plaintext() {
        ...
        let envelope = prepare_outgoing_message(
            &mut alice_mgr, &bob_peer_id, &alice_queue, plaintext,
        ).unwrap();
    ...
    fn no_session_returns_error() {
        ...
        let result = prepare_outgoing_message(
            &mut alice_mgr, &fake_peer, &queue, b"hello",
        );
    ...
    fn tampered_envelope_is_rejected() {
        ...
        let mut envelope = prepare_outgoing_message(
            &mut alice_mgr, &bob_peer_id, &alice_queue, b"tamper test",
        ).unwrap();
    ...
    fn multiple_messages_each_unique() {
        ...
        let env1 = prepare_outgoing_message(
            &mut alice_mgr, &bob_peer_id, &alice_queue, b"same text",
        ).unwrap();
        let env2 = prepare_outgoing_message(
            &mut alice_mgr, &bob_peer_id, &alice_queue, b"same text",
        ).unwrap();
